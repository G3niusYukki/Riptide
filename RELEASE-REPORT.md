...
**P0 · Survival Fixes**
- ✅ Kill Switch (On-Demand VPN) — blocks all traffic when VPN disconnected
- ✅ Sleep/Wake recovery — auto-restores runtime after Mac sleep cycles
- ✅ Network change recovery — detects WiFi/Ethernet transitions
- ✅ DNS leak prevention — `sleep()`/`wake()` overrides in PacketTunnelProvider

**P1 · Protocol Completeness**
- ✅ VLESS Reality transport layer connected (TLS camouflage, SNI, cert verification)
- ✅ WireGuard full pipeline (models → parsing → validation → mihomo config generation)
- ✅ TUIC confirmed fully implemented (TUICClient, config, parsing, validation)

**P2 · Runtime Robustness**
- ✅ Startup race eliminated — adaptive exponential backoff (100ms→2s, 10s max)
- ✅ Graceful node degradation — auto-failover to next proxy on connection failure
- ✅ LegacyTCPStateMachine removed (~155 lines of NSLock-based duplicate code)

**P3 · Observability & Operations**
- ✅ `DiagnosticReport` module — structured JSON diagnostic reports
- ✅ `GET /diagnostics` REST + WebSocket endpoints
- ✅ `ConnectionTiming` metrics — policy resolution + proxy connect latency tracking

### Stats
- **22 files changed**, +756 / −188 lines
- Build: ✅ zero errors
...
| 功能模块 | 状态 | 说明 |
|---------|------|------|
| 代理协议 | ✅ 完整 | SS/VMess/VLESS/VLESS-Reality/Trojan/Hy2/TUIC/WireGuard/Snell/SOCKS5/HTTP |
...
| Kill Switch | ✅ 完整 | On-Demand VPN 防泄漏 |
| 休眠/唤醒恢复 | ✅ 完整 | SleepWakeObserver 自动恢复 |
| 网络变化恢复 | ✅ 完整 | NWPathMonitor 自动检测 |
| 优雅降级 | ✅ 完整 | 节点故障自动切换 |
| 诊断报告 | ✅ 完整 | REST/WS `/diagnostics` 端点 |
