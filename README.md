...
> **What ships today (v2.4.0):** macOS is the primary development target with a SwiftUI app + Swift-first proxy library; Windows is a Tauri 2 port (`riptide-windows/`) sharing the same Clash YAML profile format; Linux is built by CI but not yet packaged. iOS support is **not** planned — see the archived `docs/_archive/riptide-ios-stub/`.
...
- **诊断 (Diagnostics, v2.4.0)** — persistent diagnostic event journal (Logbook) with level / category / date filters, JSONL export, and clear. Closes the gap between "what's happening right now" (Logs) and "what happened across restarts" (Diagnostics)
...
- **Override schema (v2.4.0)** — per-profile YAML overlays with `meta.replace` and `removed:` semantics, applied at runtime via `OverrideStore`
- **QR code share (v2.4.0)** — generate industry-standard share URIs for 6 protocols (ss / vmess / vless / trojan / hysteria2 / tuic); per-row QR code + batch "Share All"
...
- Diff-based closed-connection watcher (data model + algorithm in v2.4.0; runtime tick loop is the v2.4.x follow-up)
...
Riptide v2.4.0 is distributed through three channels, **with no `xattr -cr` workaround required** when the build is signed and notarized (the recommended configuration). Pick whichever fits your setup:
...
swift test --filter "LogbookStore"       # v2.4.0 Logbook suite
...
> Status reflects the v2.4.0 configuration. Items marked *(when configured)* activate automatically once the corresponding credentials are provided to the release pipeline — see [`docs/signing-setup.md`](docs/signing-setup.md).
...
- For Mac App Store submission the sandbox would need to be re-enabled and the entitlement set trimmed accordingly — see [`docs/MAC-APP-STORE-CHECKLIST.md`](docs/MAC-APP-STORE-CHECKLIST.md). v2.4.0 does **not** ship to the App Store.
...
6. **Dependency injection** over hard-coded global behavior. For v2.4.0+ cross-cutting concerns, the pattern is `Module.setFoo(dependency) async` on actors + a single injection site in `AppViewModel.init`
